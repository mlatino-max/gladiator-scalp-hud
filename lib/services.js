...
"use strict";
const { engine, alp, pagedBars, fetchOrders, fetchJournal, fetchDailyBars, isTradingDay, etOffsetFrom, TRADING_BASE, DATA_BASE, FEED } = require("./alpaca.js");
const store = require("./store.js");
const alerts = require("./alerts.js");
const sb = require("./supabase.js");
const mirror = require("./journal-mirror.js");
...
async function scan(extraSymbols) {
  ...
  /* every scan keeps the Supabase project awake (the free plan pauses after
     seven idle days); it never blocks or fails the scan */
  void sb.heartbeat("scan");
...
/* ---- journal + evidence ---- */
async function reconstruct(days) {
...
/* /api/journal: the fresh reconstruction, plus — while the Supabase mirror
   earns trust (plan phase 1) — the table's counts over the same window and
   whether they agree. A Supabase failure is reported, never fatal. */
async function journal(days) {
  const j = await reconstruct(days);
  j.mirror = await mirror.mirrorStatus(j.entries, days).catch(e => ({ configured: true, error: e.message }));
  return j;
}
async function evidence(days) {
  const [j, snaps, verdicts, cached] = await Promise.all([
    reconstruct(days || 365),
...
async function regimeSnapshot(opts) {
  ...
  void sb.heartbeat("regime-snapshot");
...
async function journalSync(opts) {
  ...
  void sb.heartbeat("journal-sync");
  ...
  /* Supabase mirror (plan phase 1): orders, fills, round_trips from the same
     fills the engine just paired. store.json on the volume is a cache; the
     tables are the durable copy. A mirror failure is alerted, not fatal. */
  if (sb.configured()) {
    try {
      const orders = await fetchOrders(365);
      run.mirror = await mirror.syncJournalMirror(orders, engine.buildRoundTrips(orders));
    } catch (e) {
      run.mirror = { ok: false, error: e.message };
      await alerts.evaluate(STRATEGY, { plumbing: { key: "journal-mirror", value: String(e.message).slice(0, 80), detail: "journal mirror failed: " + e.message } }).catch(() => {});
    }
  } else {
    run.mirror = { configured: false };
  }
...
async function opsStatus() {
  ...
  out.cron = {
    ...
    screenLoad: await store.lastCronRun("screen-load").catch(() => null),
    screen: await store.lastCronRun("screen").catch(() => null)
  };
  out.supabase = { configured: sb.configured(), host: sb.host() };
  if (out.supabase.configured) {
    try { out.supabase.ping = await sb.ping(); } catch (e) { out.supabase.ping = false; out.supabase.error = e.message; }
  }
  out.massive = { configured: massive.configured() };
  try {
...
module.exports = { STRATEGY, CORE, INDEXES, equityCap, scan, account, verdict, ctxFrom, reconstruct, journal, evidence, regimeSnapshot, journalSync, opsStatus };
