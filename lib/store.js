/* Durable state for v2: regime snapshots, persisted round trips, verdict
   snapshots, alert de-dupe. Three backends, chosen from the environment:
   a JSON file on a mounted volume when HUD_STORE_FILE is set (the local
   Docker stack), Vercel KV (Upstash REST) when KV_REST_API_URL /
   KV_REST_API_TOKEN are set (optionally under the prefix named by
   KV_ENV_PREFIX), otherwise an in-process memory map that is honest about
   being volatile (`store.kind`). */
"use strict";
const fs = require("fs");
...
/* One JSON object in one file, rewritten atomically (temp + rename) on every
   write. A single-user desk holds a few hundred keys, so this is plenty and
   it survives container restarts on the compose volume. */
class FileStore {
  constructor(file) { this.kind = "file"; this.file = file; this.map = new Map(); this.loaded = false; }
  load() {
  ...
  async get(key) { this.load(); return this.map.has(key) ? JSON.parse(this.map.get(key)) : null; }
  async set(key, value) { this.load(); this.map.set(key, JSON.stringify(value)); this.flush(); return true; }
  async del(key) { this.load(); const hit = this.map.delete(key); if (hit) this.flush(); return hit; }
  async keys(prefix) { this.load(); return [...this.map.keys()].filter(k => k.startsWith(prefix)).sort(); }
  async ping() { this.load(); fs.accessSync(path.dirname(this.file), fs.constants.W_OK); return true; }
}

/* Vercel's storage integration can inject its variables under a custom
   prefix (this project's are gladiator_scalp_KV_REST_API_URL etc.).
   KV_ENV_PREFIX names that prefix; the bare names still win when set. */
function kvEnv(name) {
  const prefix = process.env.KV_ENV_PREFIX || "";
  return process.env[name] || (prefix ? process.env[prefix + name] : undefined);
}

let _store = null;
function store() {
  if (_store) return _store;
  const file = process.env.HUD_STORE_FILE;
  const url = kvEnv("KV_REST_API_URL"), token = kvEnv("KV_REST_API_TOKEN");
  _store = file ? new FileStore(file) : url && token ? new KvStore(url, token) : new MemoryStore();
  return _store;
}
...
module.exports = {
  store, useStore, MemoryStore, KvStore, FileStore, key,
