...
function creds() {
  /* Vercel names first, then the Alpaca SDK names, then the names the
     official MCP server and the alpaca-bot .env use — one .env can feed
     every container of the local stack. */
  let key = process.env.ALPACA_API_KEY_ID || process.env.APCA_API_KEY_ID || process.env.ALPACA_API_KEY;
  let secret = process.env.ALPACA_API_SECRET_KEY || process.env.APCA_API_SECRET_KEY || process.env.ALPACA_SECRET_KEY;
  if (!key || !secret) {
...
async function alp(base, path, params) {
  ...
  if (!c) {
    ...
    err.status = 503;
    err.hint = "Set ALPACA_API_KEY_ID and ALPACA_API_SECRET_KEY (or ALPACA_API_KEY / ALPACA_SECRET_KEY) — paper keys — in the deployment env.";
    throw err;
  }
